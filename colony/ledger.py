...
def create_account(con, account_id, kind):
    ...
    con.execute("INSERT INTO balances (account_id, balance_cents) VALUES (?, 0)", (account_id,))


def balance(con, account_id):
    row = con.execute(
        "SELECT balance_cents FROM balances WHERE account_id = ?", (account_id,)
    ).fetchone()
    if row is None:
...
def transfer(con, tick, debit, credit, amount_cents, memo):
    ...
    if not isinstance(amount_cents, int) or amount_cents <= 0:
        raise LedgerError(f"bad amount {amount_cents!r} for memo {memo!r}")
    row = con.execute(
        "SELECT b.balance_cents, a.kind FROM balances b"
    ...
    bal, kind = row
    if kind != "ARENA" and bal - amount_cents < 0:
        raise InsufficientFunds(f"{debit} holds {bal}, cannot pay {amount_cents} ({memo})")
    con.execute(
        "UPDATE balances SET balance_cents = balance_cents - ? WHERE account_id = ?",
        (amount_cents, debit),
    )
    cur = con.execute(
        "UPDATE balances SET balance_cents = balance_cents + ? WHERE account_id = ?",
    ...
    con.execute(
        "INSERT INTO ledger (tick, debit_account, credit_account, amount_cents, memo)"
        " VALUES (?, ?, ?, ?, ?)",
        (tick, debit, credit, amount_cents, memo),
    )


def verify_invariants(con, initial_treasury_cents):
    ...
    ledger. Together: SUM(all balances) == initial_treasury_cents, forever.
    """
    total = con.execute("SELECT COALESCE(SUM(balance_cents), 0) FROM balances").fetchone()[0]
    if total != initial_treasury_cents:
        raise AccountingError(
            f"conservation violated: SUM(balances) = {total}, expected {initial_treasury_cents}"
        )
    rows = con.execute(
        """
        SELECT a.id, a.kind, b.balance_cents,
               COALESCE(c.s, 0) AS credits, COALESCE(d.s, 0) AS debits
        FROM accounts a
        JOIN balances b ON b.account_id = a.id
        LEFT JOIN (SELECT credit_account AS k, SUM(amount_cents) AS s FROM ledger GROUP BY 1) c
               ON c.k = a.id
        LEFT JOIN (SELECT debit_account AS k, SUM(amount_cents) AS s FROM ledger GROUP BY 1) d
               ON d.k = a.id
        """
    ).fetchall()
    for account_id, kind, bal, credits, debits in rows:
        base = initial_treasury_cents if account_id == "TREASURY" else 0
